...
## 持久化
- 默认RDB(Redis DataBase)
  - 在指定的时间间隔内，执行指定次数的写操作，则会将内存中的数据写入到磁盘中(默认是 900秒内有1个更改，300秒内有10个更改以及60秒内有10000个更改)
...
- AOF(Append Only File)
  - 默认不开启，为了弥补RDB数据不一致
  - 采用日志的形式来记录每个**写操作**，并**追加**到文件中。Redis 重启的会根据日志文件的内容将写指令从前到后执行一次以完成数据的恢复工作
  - 触发
    - 每次执行触发
    - 每秒触发(不同步)
  - 恢复：正常情况下，将appendonly.aof 文件拷贝到redis的安装目录的bin目录下，重启redis服务即可
  - 重写机制：当AOF文件的大小超过所设定的阈值时，Redis就会对AOF文件的内容压缩
    - fork出一条新进程，读取内存中的数据，并重新写到一个临时文件中。并没有读取旧文件，最后替换旧的aof文件。
  - 优点：数据完整性和一致性更高
  - 缺点：文件越来越大，数据恢复越来越慢
- 总结
